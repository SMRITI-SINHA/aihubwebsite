...
const aiChatMatch = content.match(/title: "AI Chat",\s*icon: MessageSquareText,\s*items: \[/);
if (aiChatMatch) {
...
function RobotModel() {
  ...
  return (
    <Float speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
      <primitive ref={robotRef} object={scene} scale={2} position={[0, 0, 0]} />
    </Float>
  );
}
    ...
    >
      <div className="rounded-[30px] border bg-white/55 p-2 shadow-sm backdrop-blur">
        <div className="grid gap-6 rounded-[26px] border bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(255,255,255,0.70))] p-6 md:p-8 md:grid-cols-[1fr,1.2fr] md:items-center">
          ...
          <div className="relative h-[300px] md:h-[450px] rounded-[22px] border bg-[hsl(var(--background))] overflow-hidden">
          ...
          <div className="grid gap-6">
            <div>
              <div className="text-xl font-semibold mb-2">Speak, and let AI do the work</div>
            ...
            <div className="grid gap-4">
              {[
                ...
                return (
                  <div key={idx} className="flex gap-4 p-4 rounded-2xl border bg-white/80 shadow-sm transition hover:bg-white">
                    ...
                    <div>
                      ...
                      <div className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">{feature.desc}</div>
                    </div>
                  </div>
                );
              })}
